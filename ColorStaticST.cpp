...
void CColorStaticST::OnKillFocus(CWnd* pNewWnd) 
{
...
//     memDC.DrawText(m_strText, rect, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
//     dc.BitBlt(0,0,rect.Width(),rect.Height(),&memDC,0,0,SRCCOPY);
}

void CColorStaticST::SetWindowText( LPCTSTR szText )
{   
    CDC *pDC = GetDC();
    CString str = szText;
    ...
    if (size.cx > rect.Width())
        ModifyStyle(1, SS_LEFT|BS_MULTILINE);
    else
       ModifyStyle(1, SS_LEFT|SS_CENTERIMAGE);
